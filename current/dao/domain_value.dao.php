...
class dao_arsdehnel_msub_domain_value {
	...
	//retrieve select stuff
	public function retrieve_select(){
		if( isset( $this->domain_code ) ){
			$rs	= fnc_rs_lookup( "SELECT dv.code
									,dv.description
								  FROM arsdehnel_msub.domain_value dv
								    ,arsdehnel_msub.domain d
								  WHERE dv.domain_id = d.domain_id
								    AND d.code = '" . $this->domain_code . "'
								  ORDER BY dv.description" );
		}
		return $rs;
	}
}
